...
@pytest.mark.asyncio
async def test_create_queue_async(pgmq_all_variants, async_db_session):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
...
@pytest.mark.asyncio
async def test_create_partitioned_queue_async(pgmq_all_variants, async_db_session):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
...
@pytest.mark.asyncio
async def test_create_same_queue_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE, async_db_session
):
    pgmq, queue_name = async_pgmq_setup_teardown
    await pgmq.create_queue_async(queue_name)
    assert await check_queue_exists_async(async_db_session, queue_name) is True
    await pgmq.create_queue_async(queue_name)
    # `create_queue` with the same queue name should not raise an exception
...
@pytest.mark.asyncio
async def test_validate_queue_name_async(pgmq_all_variants):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
    await pgmq.validate_queue_name_async(queue_name)
    # `queue_name` should be a less than 48 characters
    with pytest.raises(Exception) as e:
        await pgmq.validate_queue_name_async("a" * 49)
    error_msg: str = str(e.value.orig)
    assert "queue name is too long, maximum length is 48 characters" in error_msg
...
@pytest.mark.asyncio
async def test_drop_queue_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    _ = async_pgmq_setup_teardown
    pass
...
@pytest.mark.asyncio
async def test_drop_non_exist_queue_async(pgmq_all_variants, async_db_session):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
...
@pytest.mark.asyncio
async def test_drop_partitioned_queue_async(
    async_pgmq_partitioned_setup_teardown: PGMQ_WITH_QUEUE,
):
    _ = async_pgmq_partitioned_setup_teardown
    pass
...
@pytest.mark.asyncio
async def test_drop_non_exist_partitioned_queue_async(
    pgmq_all_variants, async_db_session
):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
    assert await check_queue_exists_async(async_db_session, queue_name) is False
...
@pytest.mark.asyncio
async def test_list_queues_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    queues = await pgmq.list_queues_async()
    assert queue_name in queues
...
@pytest.mark.asyncio
async def test_list_partitioned_queues_async(
    async_pgmq_partitioned_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_partitioned_setup_teardown
    queues = await pgmq.list_queues_async()
    assert queue_name in queues
...
@pytest.mark.asyncio
async def test_send_and_read_msg_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_id: int = await pgmq.send_async(queue_name, msg)
...
@pytest.mark.asyncio
async def test_send_and_read_msg_with_delay_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_id: int = await pgmq.send_async(queue_name, msg, delay=2)
    msg_read = await pgmq.read_async(queue_name)
    assert msg_read is None
...
@pytest.mark.asyncio
async def test_send_and_read_msg_with_vt_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_id: int = await pgmq.send_async(queue_name, msg)
    msg_read = await pgmq.read_async(queue_name, vt=2)
    assert msg_read.message == msg
    assert msg_read.msg_id == msg_id
...
@pytest.mark.asyncio
async def test_send_and_read_msg_with_vt_and_delay_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
...
@pytest.mark.asyncio
async def test_send_and_read_msg_with_vt_zero_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    """Test that vt=0 works correctly and message becomes visible immediately."""
    pgmq, queue_name = async_pgmq_setup_teardown
    ...
    msg_id: int = await pgmq.send_async(queue_name, msg)
    # Read with vt=0 means message should be immediately visible again
    msg_read = await pgmq.read_async(queue_name, vt=0)
    assert msg_read.message == msg
    assert msg_read.msg_id == msg_id
    # Message should be visible immediately (no waiting)
    msg_read = await pgmq.read_async(queue_name)
...
@pytest.mark.asyncio
async def test_read_empty_queue_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg_read = await pgmq.read_async(queue_name)
    assert msg_read is None
...
@pytest.mark.asyncio
async def test_read_batch_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_id_1: int = await pgmq.send_async(queue_name, msg)
    msg_id_2: int = await pgmq.send_async(queue_name, msg)
    msg_read = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_read) == 2
    assert msg_read[0].message == msg
    assert msg_read[0].msg_id == msg_id_1
    assert msg_read[1].message == msg
...
@pytest.mark.asyncio
async def test_read_batch_empty_queue_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg_read = await pgmq.read_batch_async(queue_name, 3)
    assert msg_read is None
...
@pytest.mark.asyncio
async def test_send_batch_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(
        queue_name=queue_name, messages=[msg, msg, msg]
    )
    assert len(msg_ids) == 3
    assert msg_ids == [1, 2, 3]
...
@pytest.mark.asyncio
async def test_send_batch_with_read_batch_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    ...
    assert len(msg_ids) == 3
    assert msg_ids == [1, 2, 3]
    msg_read_batch = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_read_batch) == 3
    assert [msg_read.message for msg_read in msg_read_batch] == [msg, msg, msg]
    assert [msg_read.msg_id for msg_read in msg_read_batch] == [1, 2, 3]
...
@pytest.mark.asyncio
async def test_read_with_poll_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(
        queue_name, [msg, msg, msg, msg, msg], delay=2
    )
    start_time = time.time()
    msg_reads = await pgmq.read_with_poll_async(
        queue_name,
        vt=1000,
        qty=3,
        max_poll_seconds=5,
...
@pytest.mark.asyncio
async def test_read_with_poll_with_empty_queue_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_setup_teardown
    start_time = time.time()
...
@pytest.mark.asyncio
async def test_set_vt_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_id = await pgmq.send_async(queue_name, msg)
    msg_read = await pgmq.set_vt_async(queue_name, msg_id, 2)
    assert msg is not None
    assert await pgmq.read_async(queue_name) is None
    await asyncio.sleep(1.5)
    assert await pgmq.read_async(queue_name) is None
    await asyncio.sleep(0.6)
    msg_read = await pgmq.read_async(queue_name)
...
@pytest.mark.asyncio
async def test_set_vt_to_smaller_value_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_id = await pgmq.send_async(queue_name, msg)
    _ = await pgmq.read_async(queue_name, vt=5)  # set vt to 5 seconds
    assert msg is not None
    assert await pgmq.read_async(queue_name) is None
    await asyncio.sleep(0.5)
    assert await pgmq.set_vt_async(queue_name, msg_id, 1) is not None
    await asyncio.sleep(0.3)
    assert await pgmq.read_async(queue_name) is None
    await asyncio.sleep(0.8)
    assert await pgmq.read_async(queue_name) is not None
...
@pytest.mark.asyncio
async def test_set_vt_not_exist_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg_updated = await pgmq.set_vt_async(queue_name, 999, 20)
    assert msg_updated is None
...
@pytest.mark.asyncio
async def test_pop_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    msg = await pgmq.pop_async(queue_name)
    assert msg.msg_id == msg_ids[0]
    assert msg.message == MSG
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_reads) == 2
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids[1:]
...
@pytest.mark.asyncio
async def test_pop_empty_queue_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = await pgmq.pop_async(queue_name)
    assert msg is None
...
@pytest.mark.asyncio
async def test_delete_msg_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.delete_async(queue_name, msg_ids[1]) is True
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_reads) == 2
    assert [msg_read.msg_id for msg_read in msg_reads] == [msg_ids[0], msg_ids[2]]
...
@pytest.mark.asyncio
async def test_delete_msg_not_exist_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.delete_async(queue_name, 999) is False
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_reads) == 3
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids
...
@pytest.mark.asyncio
async def test_delete_batch_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.delete_batch_async(queue_name, [msg_ids[0], msg_ids[2]]) == [
        msg_ids[0],
        msg_ids[2],
    ]
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
...
@pytest.mark.asyncio
async def test_delete_batch_not_exist_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.delete_batch_async(queue_name, [999, 998]) == []
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_reads) == 3
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids
...
@pytest.mark.asyncio
async def test_archive_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.archive_async(queue_name, msg_ids[0]) is True
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_reads) == 2
    assert [msg_read.msg_id for msg_read in msg_reads] == [msg_ids[1], msg_ids[2]]
...
@pytest.mark.asyncio
async def test_archive_not_exist_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.archive_async(queue_name, 999) is False
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_reads) == 3
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids
...
@pytest.mark.asyncio
async def test_archive_batch_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.archive_batch_async(queue_name, [msg_ids[0], msg_ids[2]]) == [
        msg_ids[0],
        msg_ids[2],
    ]
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
...
@pytest.mark.asyncio
async def test_archive_batch_not_exist_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    msg_ids = await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.archive_batch_async(queue_name, [999, 998]) == []
    msg_reads = await pgmq.read_batch_async(queue_name, 3)
    assert len(msg_reads) == 3
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids
...
@pytest.mark.asyncio
async def test_purge_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    msg = MSG
    assert await pgmq.purge_async(queue_name) == 0
    await pgmq.send_batch_async(queue_name, [msg, msg, msg])
    assert await pgmq.purge_async(queue_name) == 3
...
@pytest.mark.asyncio
async def test_metrics_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = async_pgmq_setup_teardown
    metrics = await pgmq.metrics_async(queue_name)
    assert metrics is not None
    assert metrics.queue_name == queue_name
    assert metrics.queue_length == 0
    assert metrics.newest_msg_age_sec is None
...
@pytest.mark.asyncio
async def test_metrics_all_queues_async(async_pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    # Since default PostgreSQL isolation level is `READ COMMITTED`,
    # pytest-xdist is running in **muti-process** mode, which causes **Phantom read** !
    # - `pgmq.metrics_all()` will first get the queue list, then get the metrics for each queue
    # - If another process teardown the queue before the metrics are fetched, will throw an exception that the `{queue_name}` does not exist
    with FileLock(LOCK_FILE_NAME):
        pgmq, queue_name_1 = async_pgmq_setup_teardown
        queue_name_2 = f"test_queue_{uuid.uuid4().hex}"
        await pgmq.create_queue_async(queue_name_2)
        await pgmq.send_batch_async(queue_name_1, [MSG, MSG, MSG])
        await pgmq.send_batch_async(queue_name_2, [MSG, MSG])
        metrics_all = await pgmq.metrics_all_async()
        queue_1 = [q for q in metrics_all if q.queue_name == queue_name_1][0]
        queue_2 = [q for q in metrics_all if q.queue_name == queue_name_2][0]
        assert queue_1.queue_length == 3
        assert queue_2.queue_length == 2
...
async def test_create_time_based_partitioned_queue_async(
    ...
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
    await pgmq.create_partitioned_queue_async(
        queue_name, partition_interval="1 day", retention_interval="7 days"
    )
    assert await check_queue_exists_async(async_db_session, queue_name) is True
...
@pytest.mark.asyncio
async def test_create_time_based_partitioned_queue_various_intervals_async(
    pgmq_all_variants, async_db_session
):
    pgmq: PGMQueue = pgmq_all_variants
    ...
    assert await check_queue_exists_async(async_db_session, queue_name_hour) is True
    ...
    assert await check_queue_exists_async(async_db_session, queue_name_week) is True
...
@pytest.mark.asyncio
async def test_create_partitioned_queue_invalid_time_interval_async(pgmq_all_variants):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
    with pytest.raises(ValueError) as e:
        await pgmq.create_partitioned_queue_async(
...
@pytest.mark.asyncio
async def test_create_partitioned_queue_invalid_numeric_interval_async(
    pgmq_all_variants,
):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
...
@pytest.mark.asyncio
async def test_read_with_poll_without_vt_async(
    async_pgmq_setup_teardown: PGMQ_WITH_QUEUE,
):
    """Test read_with_poll when vt parameter is not provided (None)."""
    ...
    # Call read_with_poll with vt=None to test the fallback logic
    # When vt is None, it should fall back to using pgmq.vt value (100)
    msgs = await pgmq.read_with_poll_async(
        queue_name,
        vt=None,  # Explicitly passing None to test the None fallback logic
        qty=1,
        max_poll_seconds=2,
...
@pytest.mark.asyncio
async def test_execute_operation_with_provided_sync_session_async(
    pgmq_by_session_maker, get_async_session_maker, async_db_session
):
    """Test _execute_operation sync path when session is provided."""
    ...
    async with get_async_session_maker() as session:
        # Create queue with provided session
        await pgmq.create_queue_async(queue_name, session=session)

        # Verify queue was created
        assert await check_queue_exists_async(async_db_session, queue_name) is True

        # Send a message with the same provided session
        msg_id = await pgmq.send_async(queue_name, MSG, session=session)

        # Read message with the same provided session
        msg = await pgmq.read_async(queue_name, vt=30, session=session)
        ...
        # Clean up with the same provided session
        await pgmq.drop_queue_async(queue_name, session=session)
