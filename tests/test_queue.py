...
from tests._async_utils import call_method
...
def test_create_queue(pgmq_all_variants, db_session):
    ...
    call_method(pgmq, "create_queue", queue_name)
...
def test_create_partitioned_queue(pgmq_all_variants, db_session):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
    call_method(pgmq, "create_partitioned_queue", queue_name)
    assert check_queue_exists(db_session, queue_name) is True
...
def test_validate_queue_name(pgmq_all_variants):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
    call_method(pgmq, "validate_queue_name", queue_name)
    # `queue_name` should be a less than 48 characters
    with pytest.raises(Exception) as e:
        call_method(pgmq, "validate_queue_name", "a" * 49)
...
def test_drop_non_exist_queue(pgmq_all_variants, db_session):
    ...
    with pytest.raises(ProgrammingError):
        call_method(pgmq, "drop_queue", queue_name)
...
def test_drop_non_exist_partitioned_queue(pgmq_all_variants, db_session):
    ...
    assert check_queue_exists(db_session, queue_name) is False
    with pytest.raises(ProgrammingError):
        call_method(pgmq, "drop_queue", queue_name, partitioned=True)
...
def test_send_and_read_msg(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    msg_id: int = call_method(pgmq, "send", queue_name, msg)
    msg_read = call_method(pgmq, "read", queue_name)
...
def test_send_and_read_msg_with_delay(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    msg_id: int = call_method(pgmq, "send", queue_name, msg, delay=2)
    msg_read = call_method(pgmq, "read", queue_name)
...
def test_send_and_read_msg_with_vt(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_id: int = call_method(pgmq, "send", queue_name, msg)
    msg_read = call_method(pgmq, "read", queue_name, vt=2)
    assert msg_read.message == msg
    assert msg_read.msg_id == msg_id
    time.sleep(1.5)
    msg_read = call_method(pgmq, "read", queue_name)
...
def test_send_and_read_msg_with_vt_zero(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg_read = call_method(pgmq, "read", queue_name, vt=0)
    assert msg_read.message == msg
    assert msg_read.msg_id == msg_id
...
def test_read_batch(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    msg_id_1: int = call_method(pgmq, "send", queue_name, msg)
    msg_id_2: int = call_method(pgmq, "send", queue_name, msg)
    msg_read = call_method(pgmq, "read_batch", queue_name, 3)
...
def test_send_batch(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name=queue_name, messages=[msg, msg, msg])
...
def test_send_batch_with_read_batch(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    assert len(msg_ids) == 3
    assert msg_ids == [1, 2, 3]
    msg_read_batch = call_method(pgmq, "read_batch", queue_name, 3)
...
def test_read_with_poll(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg, msg, msg], delay=2)
    start_time = time.time()
    msg_reads = call_method(pgmq, "read_with_poll", 
        queue_name,
        vt=1000,
        qty=3,
        max_poll_seconds=5,
...
def test_set_vt(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    msg_id = call_method(pgmq, "send", queue_name, msg)
    msg_read = call_method(pgmq, "set_vt", queue_name, msg_id, 2)
    assert msg is not None
    assert call_method(pgmq, "read", queue_name) is None
    time.sleep(1.5)
    assert call_method(pgmq, "read", queue_name) is None
    time.sleep(0.6)
    msg_read = call_method(pgmq, "read", queue_name)
...
def test_set_vt_to_smaller_value(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_id = call_method(pgmq, "send", queue_name, msg)
    _ = call_method(pgmq, "read", queue_name, vt=5)  # set vt to 5 seconds
    assert msg is not None
    assert call_method(pgmq, "read", queue_name) is None
    time.sleep(0.5)
    assert call_method(pgmq, "set_vt", queue_name, msg_id, 1) is not None
    time.sleep(0.3)
    assert call_method(pgmq, "read", queue_name) is None
    time.sleep(0.8)
    assert call_method(pgmq, "read", queue_name) is not None


def test_set_vt_not_exist(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg_updated = call_method(pgmq, "set_vt", queue_name, 999, 20)
...
def test_pop(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    msg = call_method(pgmq, "pop", queue_name)
    assert msg.msg_id == msg_ids[0]
    assert msg.message == MSG
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
...
def test_delete_msg(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "delete", queue_name, msg_ids[1]) is True
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
    assert len(msg_reads) == 2
...
def test_delete_msg_not_exist(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "delete", queue_name, 999) is False
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
    assert len(msg_reads) == 3
...
def test_delete_batch(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "delete_batch", queue_name, [msg_ids[0], msg_ids[2]]) == [
        msg_ids[0],
        msg_ids[2],
    ]
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
...
def test_delete_batch_not_exist(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "delete_batch", queue_name, [999, 998]) == []
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
    assert len(msg_reads) == 3
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids
...
def test_archive(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "archive", queue_name, msg_ids[0]) is True
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
    assert len(msg_reads) == 2
...
def test_archive_not_exist(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "archive", queue_name, 999) is False
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
    assert len(msg_reads) == 3
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids
...
def test_archive_batch(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "archive_batch", queue_name, [msg_ids[0], msg_ids[2]]) == [
        msg_ids[0],
        msg_ids[2],
    ]
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
...
def test_archive_batch_not_exist(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msg = MSG
    msg_ids = call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "archive_batch", queue_name, [999, 998]) == []
    msg_reads = call_method(pgmq, "read_batch", queue_name, 3)
    assert len(msg_reads) == 3
    assert [msg_read.msg_id for msg_read in msg_reads] == msg_ids
...
def test_purge(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    msg = MSG
    assert call_method(pgmq, "purge", queue_name) == 0
    call_method(pgmq, "send_batch", queue_name, [msg, msg, msg])
    assert call_method(pgmq, "purge", queue_name) == 3


def test_metrics(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    pgmq, queue_name = pgmq_setup_teardown
    metrics = call_method(pgmq, "metrics", queue_name)
...
def test_create_time_based_partitioned_queue(pgmq_all_variants, db_session):
    pgmq: PGMQueue = pgmq_all_variants
    queue_name = f"test_queue_{uuid.uuid4().hex}"
    call_method(pgmq, "create_partitioned_queue", 
...
def test_read_with_poll_without_vt(pgmq_setup_teardown: PGMQ_WITH_QUEUE):
    ...
    msgs = call_method(pgmq, "read_with_poll", 
        queue_name,
...
def test_execute_operation_with_provided_sync_session(pgmq_by_session_maker, get_session_maker, db_session):
    ...
    with get_session_maker() as session:
        ...
        call_method(pgmq, "create_queue", queue_name, session=session)
        
        # Verify queue was created
        assert check_queue_exists(db_session, queue_name) is True
        
        # Send a message with the same provided session
        msg_id = call_method(pgmq, "send", queue_name, MSG, session=session)
        
        # Read message with the same provided session
        msg = call_method(pgmq, "read", queue_name, vt=30, session=session)
        ...
        call_method(pgmq, "drop_queue", queue_name, session=session)
...
def test_execute_operation_async_with_session_none(pgmq_by_async_dsn, db_session):
    ...
    call_method(pgmq, "create_queue", queue_name)
    msg_id = call_method(pgmq, "send", queue_name, MSG)
    msg = call_method(pgmq, "read", queue_name, vt=30)
